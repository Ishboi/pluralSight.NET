  ...
  class Program
  {
    static void Main(string[] args)
    {
      ...
      foreach (var session in speaker.Sessions)
      {
        ...
        var doesSpeakerHaveEarlierSessions = DoesSpeakerHaveEarlierSessions(speaker, session);
        ...
        if(timeUntilNextSession != TimeSpan.MinValue && session.ScheduledAt.Add(session.Length).DayOfYear != session.ScheduledAt.Add(session.Length).Add(timeUntilNextSession).DayOfYear)
        {
    ...
    public static bool DoesSpeakerHaveEarlierSessions(Speaker speaker, Session currentSession)
    {

      return speaker.Sessions.Any(
          session => session.Id != currentSession.Id && session.ScheduledAt.CompareTo(currentSession.ScheduledAt) == (int)DateComparison.Earlier
        );
    }
