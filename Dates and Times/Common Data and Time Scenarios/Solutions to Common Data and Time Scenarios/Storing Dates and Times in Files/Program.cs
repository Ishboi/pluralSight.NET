...
namespace Storing_Dates_and_Times_in_Files
{
  class Program
  {
    static void Main(string[] args)
    {
      ...
      {
        csv += $"{session.Title},{speaker.Name},{session.Length},{session.ScheduledAt.ToOffset(userOffset):O}{session.ScheduledAt:O}{Environment.NewLine}";

        File.WriteAllText($"{speaker.Name}", csv);
      }
    }
